...
export default function ServiceCard({ service, variant = 'dark' }: ServiceCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-6 flex-grow flex flex-col">
        ...
        <div className="mt-auto space-y-4">
          <Link
            ...
            className="inline-flex items-center gap-2 text-sm font-bold border-b border-current pb-0.5 hover:opacity-70 transition-opacity"
          >
      ...
      <div
        className={`absolute inset-0 ${bgClass} p-8 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-in-out z-10 flex flex-col`}
      >
        ...
        <ul className="space-y-2 overflow-y-auto scrollbar-hide flex-grow">
          ...
          {service.features.length > 5 && (
            <li className="text-xs opacity-60 italic">+ more</li>
          )}
        </ul>
        <Link
          href={`/services/${service.id}`}
          className="mt-4 w-full py-3 bg-black/10 rounded-xl flex items-center justify-center font-bold text-sm hover:bg-black/20 transition-colors"
        >
          View Details
        </Link>
      </div>
