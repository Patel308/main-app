...
const leadData = [
  ...
  { name: 'Thu', leads: 12 }, { name: 'Fri', leads: 9 }, { name: 'Sat', leads: 3 }, { name: 'Sun', leads: 2 },
];
const serviceData = [
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            { label: 'Demos Booked', value: '28', change: '+5%' },
            { label: 'Conversion Rate', value: '19.7%', change: '+2.1%' },
            { label: 'Pending Tasks', value: '5', change: '-1' },
          ].map((stat, i) => (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  {['Name','Email','Service','Status'].map(h => <th key={h} className="text-left py-3 px-4 text-sm font-semibold text-slate-600">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {[
                  { name: 'John Doe', email: 'john@example.com', service: 'Digital Marketing', status: 'New' },
                  { name: 'Jane Smith', email: 'jane@example.com', service: 'Web Development', status: 'Contacted' },
                  { name: 'Mike Johnson', email: 'mike@example.com', service: 'SEO', status: 'Qualified' },
                ].map((lead, i) => (
                  <tr key={i} className="border-b border-slate-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${lead.status === 'New' ? 'bg-blue-100 text-blue-700' : lead.status === 'Contacted' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                        {lead.status}
                      </span>
                    </td>
                  </tr>
